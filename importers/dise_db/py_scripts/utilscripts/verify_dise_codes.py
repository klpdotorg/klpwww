...
rootdir = '/home/megha/www/reports/dise_db/data/'
filelist = ['North_basic.csv','South_basic.csv','Rural_basic.csv']
#reconlist = ['Manual_Recon.csv','Manual_Recon2.csv']
reconlist = ['Manual_Recon2.csv']
correctedcsv = '/home/megha/www/reports/dise_db/py_scripts/utilscripts/Verify_dise.lst'
updatesql = '../../db_scripts/load/update_dise_codes.sql'
...
def correctDiseIds():
  ...
  for row in result:
    ...
    if len(name_in_dict) > 0:
      ...
      if row[5]:
        if row[5].strip() == dise_list[name_in_dict[0]][0]:
          ...
          if len(dist)>0 and len(blk)>0 and len(clust)>0:
            updatefile.write('UPDATE tb_school_dise SET dise_code=\'' + dise_list[name_in_dict[0]][0] + '\' where sid=' + str(row[3]) + ';\n')
            writestr = writestr + '|C'
          else:
            writestr = writestr + '|N'
      else:
      ...
      count = count + 1
      print "Processing school count:" + str(count)
    ...
    for row in reader:
      if len(row[1].strip())>0:
        if row[1].strip().isdigit():
          updatefile.write('UPDATE tb_school_dise SET dise_code=\'' + row[1] + '\' where sid=' + str(row[0]) + ';\n')
    readfile.close()
  updatefile.close()
