...
pidAyid={'1':90,'2':1,'3':2,'5':119,'6':119,'7':119,'8':119,'9':119,'10':119,'11':119,'12':119,'13':119,'14':101,'15':101,'18':101,'19':101}
...
{'tb_programme':'select id,name,"startDate","endDate",programme_institution_category_id from schools_programme where id in (1,2,3,5,6,7,8,9,14,15,18,19)'},
{'tb_assessment':'select ass.id,ass.name,ass.programme_id,ass."startDate",ass."endDate" from schools_assessment ass,schools_programme p where ass.programme_id=p.id and p.id in (1,2,3,5,6,7,8,9,14,15,18,19)'},
{'tb_question':'select q.id,q.assessment_id,q.name,q."questionType",q."scoreMax",q."scoreMin",q.grade from schools_question q, schools_assessment ass,schools_programme p where q.assessment_id=ass.id and ass.programme_id=p.id and p.id in (1,2,3,5,6,7,8,9,14,15,18,19)'},
{'tb_student_eval':'select se.question_id,se.student_id,se."answerScore",se."answerGrade" from schools_answer se,schools_question q, schools_assessment ass,schools_programme p where se.question_id=q.id and q.assessment_id=ass.id and ass.programme_id=p.id and p.id in (1,2,3,5,6,7,8,9,14,15,18,19)'},
