DROP TABLE tb_angpre_2012_agg;
CREATE TABLE tb_angpre_2012_agg
(
    sid integer,
    gender varchar(8),
    stucount integer,
...
CREATE OR REPLACE function agg_angpre2012_agg() returns void as $$
  declare
       agg RECORD;
  begin
       for agg in
       ...
       loop
         insert into tb_angpre_2012_agg (sid,gender,stucount,min_score,max_score,median_score,avg_score) values (agg.sid,agg.gender,agg.stucount,agg.min_score,agg.max_score,agg.median_score,agg.avg_score);
       end loop;
  end;
$$ language plpgsql;

select agg_angpre2012_agg() ;
