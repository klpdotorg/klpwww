...
statements = {'get_district':"select bcoord.id_bndry,ST_AsText(bcoord.coord),initcap(b.name) from vw_boundary_coord bcoord, tb_boundary b where bcoord.type='District' and b.id=bcoord.id_bndry order by b.name",
	      ...
	      'get_dise_stuinfo':"select vdi.boys_count,vdi.girls_count from vw_dise_info vdi,tb_school s where s.dise_code=vdi.dise_code and s.id=%s;",
              ...
              'get_oneteacher':"select inst.instid, ST_AsText(inst.coord), upper(s.name), s.cat, di.teacher_count, di.classroom_count, di.boys_count, di.girls_count, case when di.has_library=1 then 'yes' else 'no' end as has_library,case when di.has_ramps=1 then 'yes' else 'no' end as has_ramps, case when di.toilet_count > 0 then 'yes' else 'no' end as has_toilet, case when di.hm_count > 0 then 'yes' else 'no' end as has_hm, (di.boys_count + di.girls_count)/di.teacher_count as ptr from vw_inst_coord inst, vw_dise_info di, tb_school s,tb_boundary b where s.id=inst.instid and s.dise_code = di.dise_code and s.bid=b.id and b.type='1' and di.teacher_count = 1 and di.teacher_count>0 order by s.name",
    ...
    try:
      ...
      for row in result:
        try:
          ...
          continue
        coord = [float(match.group(1)), float(match.group(2))]
        feature = geojson.Feature(id=row[0], geometry=geojson.Point(coord), properties={"name":row[2], "cat":row[3],"teacher_count":row[4],"class_count":row[5],"boys_count":row[6],"girls_count":row[7],"has_library":row[8],"has_ramps":row[9],"has_toilets":row[10],"has_hm":row[11],"ptr":row[12]})
        features.append(feature)
      feature_collection = geojson.FeatureCollection(features)
