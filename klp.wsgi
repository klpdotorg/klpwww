...
statements = {'get_district':"select bcoord.id_bndry,ST_AsText(bcoord.coord),initcap(b.name) from vw_boundary_coord bcoord, tb_boundary b where bcoord.type='District' and b.id=bcoord.id_bndry order by b.name",
              ...
              'get_school':"(select inst.instid, ST_AsText(inst.coord), upper(s.name), s.cat from vw_inst_coord inst, tb_school s,tb_boundary c, tb_boundary b, tb_boundary d where s.id=inst.instid and s.bid=c.id and c.parent = b.id and b.id in (503,9038,502,9040) order by s.name) UNION (select inst.instid, ST_AsText(inst.coord), upper(s.name), s.cat from vw_inst_coord inst, tb_school s,tb_boundary c, tb_boundary b, tb_boundary d where s.id=inst.instid and s.bid=c.id and c.parent = b.id and b.parent = d.id and d.id in (419,420) and d.type='1' order by s.name)",
              ...
              'get_preschool':"select inst.instid ,ST_AsText(inst.coord),upper(s.name) from vw_inst_coord inst,tb_school s,tb_boundary c, tb_boundary b, tb_boundary d where s.id=inst.instid and s.bid=c.id and c.parent = b.id and b.id in (503,9038,502,9040)  and b.parent = d.id and d.id in (9064,9041,420) and d.type='2' order by s.name",
